...
# OK, getting the chromosome start and the chromosome end here. First step from snippet given in webref above.
use warnings;
use strict;
use Data::Dumper;
use Bio::Tools::GFF;
...
my ($gffio, $feature, $t, $t2, $ti, $ts, $te, $val);
my $nfeats=0;
...
while ($feature = $gffio->next_feature()) { # each feature is a line ... no connection between them, unless you match ID.
	...
	$ti=$feature->seq_id();
	# NOTE: $feature->location; is a Bio::Location::Simple object, not Bio::LocationI
